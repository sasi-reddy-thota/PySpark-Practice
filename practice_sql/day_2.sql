/**
problem statement: https://leetcode.ca/2024-01-10-2995-Viewers-Turned-Streamers/#2995-viewers-turned-streamers
...
with first_session_cte as(
    select
        *
        ,first_value(session_type) over(PARTITION BY user_id ORDER BY session_start) as first_session
    from `Sessions`
)
SELECT user_id,count(first_session) as sessions_count FROM first_session_cte
WHERE first_session='Viewer' and session_type='Streamer'
GROUP BY user_id
